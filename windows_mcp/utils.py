...
def validate_coordinates(x: Optional[int] = None, y: Optional[int] = None,
    ...
    if x is not None:
        if not isinstance(x, int):
            return False, f"X coordinate must be an integer, got {type(x)}"
        if x < 0 or x > screen_width:
            return False, f"X coordinate {x} out of bounds (0-{screen_width})"

    if y is not None:
        if not isinstance(y, int):
            return False, f"Y coordinate must be an integer, got {type(y)}"
        if y < 0 or y > screen_height:
            return False, f"Y coordinate {y} out of bounds (0-{screen_height})"
...
def validate_label(label: int, max_label: int) -> tuple[bool, Optional[str]]:
    ...
    Args:
        ...
        max_label: Maximum valid label

    Returns:
        Tuple of (is_valid, error_message)
    """
    if not isinstance(label, int):
        return False, f"Label must be an integer, got {type(label)}"
...
def validate_string(value: Any, name: str, min_length: int = 0,
    ...
    if not isinstance(value, str):
        return False, f"{name} must be a string, got {type(value)}"
...
def create_error_response(error_msg: str, tool_name: str = "") -> list[TextContent]:
    ...
    Returns:
        List with TextContent error message
    """
    if tool_name:
...
def create_success_response(msg: str, additional_content: Optional[list] = None) -> list:
    ...
    Args:
        msg: Success message
        additional_content: Optional additional content (images, etc.)

    Returns:
        List with response content
    """
    logger.info(msg)
...
def sanitize_file_path(path: str, allowed_extensions: Optional[set[str]] = None) -> tuple[bool, Optional[str], Optional[str]]:
    ...
    if not isinstance(path, str):
        return False, None, f"Path must be a string, got {type(path)}"
    ...
    if allowed_extensions:
        ...
        if ext.lower() not in allowed_extensions:
            return False, None, f"Invalid file extension: {ext}. Allowed: {allowed_extensions}"
